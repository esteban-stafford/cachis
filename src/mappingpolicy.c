...
void set_associative(Computer *computer, int cacheLevel, struct memOperation *operation, MappingResult *result){
      ...
      shift = (computer->cache[cacheLevel].offset_bits + computer->cache[cacheLevel].set_bits);
      ...
      non_masked_address = (operation->address >> computer->cache[cacheLevel].offset_bits);
      result->set = non_masked_address  & ((1 << computer->cache[cacheLevel].set_bits)-1);

      // The offset is calculated by applying an AND to the address so that the rightmost bits (the offset or palabra) are kept
      result->offset = (operation->address & ((1 << computer->cache[cacheLevel].offset_bits)-1)) >> 2;
}
...
void fully_associative(Computer *computer, int cacheLevel, struct memOperation *operation, MappingResult *result){
      ...
      result->tag = operation->address >> computer->cache[cacheLevel].offset_bits;
